export default async function handler(req, res) {
  ...
  try {
    ...
    const info = `المشروع: ${item.projectName} | الموقع: ${item.location||'جامعة أم القرى'} | النوع: ${item.projectType} | الوصف: ${item.description} | المبررات: ${item.justification||''} | التكلفة: ${fmtn(cn)} ريال | الإشراف: ${fmtn(sn)} ريال | الإجمالي: ${fmtn(tot)} ريال | المدة: ${item.duration} شهر | المستفيد: ${item.beneficiary||'منسوبو الجامعة'}`;
    ...
    const call = async (q) => {
      const r = await fetch('https://api.anthropic.com/v1/messages',{method:'POST',headers:H,body:JSON.stringify({model:'claude-sonnet-4-20250514',max_tokens:3000,system:SYS,messages:[{role:'user',content:q}]})});
      const d = await r.json();
      return d.content ? d.content.map(b=>b.text||'').join('') : '';
    };
    const [t1, t2] = await Promise.all([call(q1), call(q2)]);
    item.status = 'generated';
    item.generatedData = { t1, t2, cn, sn, tot, opx, grd, dn };
    list[idx] = item;
    await fetch(`https://api.jsonbin.io/v3/b/${BID}`, { method:'PUT', headers:{'Content-Type':'application/json','X-Master-Key':BK}, body: JSON.stringify({ requests: list }) });
