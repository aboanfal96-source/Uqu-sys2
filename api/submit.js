export default async function handler(req,res){
  ...
  const BID=process.env.JSONBIN_BIN_ID,BK=process.env.JSONBIN_API_KEY;
  ...
  try{
    ...
    await fetch(`https://api.jsonbin.io/v3/b/${BID}`,{method:'PUT',headers:{'Content-Type':'application/json','X-Master-Key':BK},body:JSON.stringify({requests:list.slice(0,200)})});
